AddEventHandler('chatMessage', function(playerId, playerName, message)
	...
	if string.sub(message, 1, string.len('/')) ~= '/' then
		...
		TriggerClientEvent('chat:addMessage', -1, {args = {Lang:t("ooc_prefix", playerName), message}, color = {0, 0, 0}})
	end
end)
...
RegisterCommand('twt', function(playerId, args, rawCommand)
	...
	if playerId == 0 then
		...
		TriggerClientEvent('chat:addMessage', -1, {
			args = {Lang:t("twt_prefix", playerName), args},
			color = {0, 153, 204}}
		)
	end
end, false)
...
RegisterCommand('ad', function(playerId, args, rawCommand)
	...
	if playerId == 0 then
		...
		TriggerClientEvent('chat:addMessage', -1, {args = {Lang:t("ad_prefix", playerName), args}, color = {0, 181, 26}})
	end
end, false)
...
RegisterCommand('anontwt', function(playerId, args, rawCommand)
	...
	if playerId == 0 then
		...
		TriggerClientEvent('chat:addMessage', -1, {args = {Lang:t("twt_prefix", "Anonymous"), args}, color = {0, 153, 204}})
	end
end, false)
...
RegisterCommand('me', function(playerId, args, rawCommand)
	if playerId == 0 then
		...
		TriggerClientEvent('esx_rpchat:sendProximityMessage', -1, playerId, Lang:t("me_prefix", playerName), args, {255, 0, 0})
	end
end, false)
...
RegisterCommand('do', function(playerId, args, rawCommand)
	if playerId == 0 then
		...
		TriggerClientEvent('esx_rpchat:sendProximityMessage', -1, playerId, Lang:t("do_prefix", playerName), args, {0, 0, 255})
	end
end, false)
